...
netacad.settings = new function() {
    ...
    this.init = function(id) {
        ...
        queryKeys["display"] = this.DISPLAY_KEY;
...
netacad.views.View.prototype.buildItemList = function(items, message) {
    ...
    if (items && items.length) {
        ...
        //Tab order going out of overlay fix
        var anchor = this.document.createElement("a");
        anchor.style.backgroundColor = "transparent";
        anchor.style.pointerEvents = "none";
        anchor.style.border = "none";
        anchor.setAttribute("href", "#");
        anchor.appendChild(this.document.createTextNode(message));
        li.appendChild(anchor);
        //------
        ul.appendChild(li);
    }
...
netacad.views.CourseIndexView.prototype.buildModuleList = function(modules, filter) {
    ...
    if (empty) {
        ...
        if (filter("type") == "PT") {
            p.appendChild(this.document.createTextNode(netacad.lang.get("NoPT")));
        } else if (filter("type") == "VIDEO") {
            p.appendChild(this.document.createTextNode(netacad.lang.get("NoVideo")));
        } else if (filter("type") == "LAB") {
            p.appendChild(this.document.createTextNode(netacad.lang.get("NoLab")));
        } else {
...
netacad.components.ScrollPane = function(element) {
    ...
    //Accessibility fix -- click event on keyboard press
    this.downButton.addEventListener("click", function(event) {

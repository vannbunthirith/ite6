...
(AnimationControl = function(obj) {
    ...
    function createButton(norm, over, callback, toolTip, xpos, name, tIndex) {
        ...
        //add discernible text
        var ariaLabelText = name;
        if (name == "play") {
            ariaLabelText = $.trim(getMediaText("disc-play"));
        } else if (name == "start") {
            ariaLabelText = $.trim(getMediaText("disc-start"));
        } else if (name == "forward") {
            ariaLabelText = $.trim(getMediaText("disc-fforward"));
        } else if (name == "end") {
            ariaLabelText = $.trim(getMediaText("disc-end"));
        } else if (name == "pause") {
            ariaLabelText = $.trim(getMediaText("disc-pause"));
        } else if (name == "rewind") {
            ariaLabelText = $.trim(getMediaText("disc-rewind"));
        }

        divElement.setAttribute("aria-label", ariaLabelText);
    ...
    function loadAnimAssets() {
        if (animAssetsLoaded == false) {
            $("#htmlMedia").last().append('<div id="animationContent"></div>');
...
(DropContainer = function(compObj) {
    ...
    if (compObj.dropIcon) {
        ...
        // console.log("coming to dropIcon")
        if (compObj.dragAlpha == 0.3) {
            ...
            var bg = new createjs.Shape();
            bg.graphics.beginLinearGradientFill(["#f4f3f4", "#ffffff"], [0, 1], 0, 0, 0, compObj.height).beginLinearGradientStroke(["#a1a4a4", "#d7dbdb"], [0, 0.9], 0, 0, 0, compObj.height).setStrokeStyle(1.3, 'round', 'round').drawRoundRect(0, 0, compObj.width, compObj.height, 1);
            t.addChild(bg);
        } else {
            var bg = new createjs.Shape();
            bg.graphics.beginLinearGradientFill(["#acaeae", "#cbdad9", "#e5e9e9"], [0, 0.5, 0.9], 0, 0, 0, compObj.height).beginLinearGradientStroke(["#747273", "#a6b8b7"], [0, 0.3], 0, 0, 0, compObj.height).setStrokeStyle(0.4).drawRoundRect(0, 0, compObj.width, compObj.height, 1);
...
(DragContainer = function(compObj, th) {
    ...
    if (!compObj.dragIcon) {
        ...
        label.y = (compObj.height - label.height()) / 2 + 3; // take label height from textComp height return func
...
(McTable = function(o, xmlNode) {
    ...
    t.createButton = function(_w, _h, _p, _k) {
        ...
        _btn.onClick = function(e) {
            // console.log(this, ' ::: ', _k, ' ::: ', e.target.id);
            _t.onoffClicked(this, _k);
    ...
    var loaded = function(_obj) {
        ...
        //Sumanth: commented above line and added below line to fix the accessibility tab index
        hdrTxt1.style.cssText = 'color:#393536;font-weight:bold;font-size:11px;padding:5px 10px 0 4px;display:inline-block;' + styleHead;
        ...
        /* Sumanth: Added below lines of code to fix the accessibility tab index */
        var colsHolder = document.createElement('div');
        colsHolder.style.cssText = 'float:right;display:inline-block;';
        $('#hrdDiv').append(colsHolder);
        /* Sumanth: Code Ends */

        for (var k = 1; k <= parseInt(tableCols); k++) {
            var styleHead = "";
            var styleHeadLast = "border-right:2px solid #fff;";
            var styleHeadCur = "";

            if (k == tableCols) {
                styleHead = "border:none;";
                styleHeadLast = ""
            } else if (k == 1) {
                styleHead = "border-right:2px solid #fff;";
            } else {
                styleHead = "border-right:2px solid #fff;";
            }
            ...
            //Sumanth: commented above line and added below line to fix the accessibility tab index
            hdrTxt.style.cssText = 'color:#393536;font-weight:normal;font-size:11px;padding:5px 12px 0 10px;display:inline-block;' + styleHead;
            $(hdrTxt).html(colsTitle);
            // $("#hrdDiv").append(hdrTxt);
            //Sumanth: commented above line and added below line to fix the accessibility tab index
            colsHolder.append(hdrTxt);
        ...
        for (i = 1; i <= tableRows; i++) {
            ...
            /*Sumanth: Added the below block of code to get the btn values on click using keyboard */
            var divHolder = document.createElement('div');
            divHolder.id = "colBtnHolder" + i
            ...
            for (var j = parseInt(tableCols); j > 0; j--) {
                ...
                var _w = w - 1;
                var _h = rowHts[i] >= maxRowHt ? rowHts[i] - 2 : maxRowHt - 2;
                var hit = t.createButton(_w, _h, q, i, t);
                hit.x = _x - w + 2;
                // console.log(hit.x);
                _x -= w + 2;
                hit.y = 0;
            ...
            $('#canvasDiv').append(divHolder);
            _textComp.x = 5;
            //_textComp.y = maxRowHt + (i*10);
            //_textComp.y = 6;
            q.addChild(_textComp);

            // var colBtnholder = document.createElement("div");
            // colBtnholder.tabIndex = 0;
            // colBtnholder.id = "colBtnholder"+i;
            // colBtnholder.style.cssText = "padding: 0;border: none;background-color: transparent;top:1px;left:"+ yPos +"px;width:" + _w + "px;height:" + (rowHts[i] >= maxRowHt ? rowHts[i] + 2 : maxRowHt + 2) + "px;visibility:hidden;pointer-events:none;";
            //
            // $("#canvasDiv").append(colBtnholder);
        ...
        var getColumnName = function(str) {
            if (str.indexOf("<b>") > -1) {
        ...
        /*Sumanth: Added the below block of code to get the btn values on click using keyboard */
        for (var k = 0; k < hitBtnArr.length; k++) {
            for (var a = hitBtnArr[k].length; a > 0; a--) {
                var divElement = document.createElement("button");
                divElement.tabIndex = 0;
                divElement.id = a - 1;
                divElement.name = getColumnName(getMediaText("txt_col" + ((hitBtnArr[k].length - a) + 1))); //giving btn name as col name to JAWS reader; //a-1;
                divElement.setAttribute('data-rownum', k + 1);
                $(divElement).attr("aria-label", "Check here button");
                divElement.onclick = function(e) {
                    var rowNum = this.getAttribute('data-rownum');
                    var btn = hitBtnArr[rowNum - 1][this.id]._btn;
                    btn.onClick()
                }
                divElement.style.cssText = "position:absolute;padding: 0;border: none;background-color: transparent;top:" + hitBtnArr[k][a - 1].yPos + "px;left:" + (hitBtnArr[k][a - 1].x + 2) + "px;width:" + (hitBtnArr[k][a - 1].width) + "px;height:" + (hitBtnArr[k][a - 1].height) + "px;pointer-events:none;";
                $('#colBtnHolder' + (k + 1)).append(divElement);
            }
        }
        /*Sumanth Code Ends*/
    ...
    var loaded = function(_obj) {
        ...
        for (var k = parseInt(tableCols); k > 0; k--) {
            ...
            if (k == tableCols) {
                styleHead = "border:none;";
                styleHeadLast = ""
            } else if (k == 1) { styleHead = "border-right:2px solid #fff;"; } else { styleHead = "border-right:2px solid #fff;"; }
...
(DropDown = function(o, xmlNode) {
     var countDrop;
    if (o.occurance == undefined) {
        countDrop = 1;
    } else {
        countDrop = o.occurance;
    }
    ...
    dropdownDiv.id = "dropdownDiv" + countDrop;
    ...
    $(xmlNode).find('subelement').each(function() {
        ...
        switch (type) {
            ...
            case "QUESTION":
                ...
                var label = getCJSElement("TEXT", { width: 450, div: "dropdownDiv" + countDrop, compId: compObj.compId, align: "left", color: "#FFFFFF", size: 11, expand: "up+down", x: compObj.x, y: compObj.y });
                t.addChild(label);
                ...
                var temp = "<label><select class='" + "select" + "' id='" + count_dropdown + "' style = float:left;width:" + 130 + "px>";
                ...
                temp += "</select></label><img src='../../../common/images/wrong-mctable.png' alt='incorrect' id='highlight_" + count_dropdown + "' style=width:0px;height:0px;float:right;margin-left:5px;></img>"
